...
First we will test getRMSE without specifying behavior to deal with the missing values. We will save the results of this test as `RMSE_NA_no_specs`.

```{r run getRMSE with resp_NA}

RMSE_NA_no_specs <- getRMSE(resp_NA, pred)

list("RMSE_without_NA_specs" = RMSE_NA_no_specs)
...
Next, we will use the code provided to again generate vectors ` resp` and `pred`. Seeing that this is the same code we used in Step 2, and that we did not alter these vectors in Step 2.2, we can safely pass these vectors through our`getMAE` function.
...
Now that we have established that our `getRMSE` and `getMAE` functions work properly, we will use them as helper functions inside of a wrapper function `getRMSE_MAE` that can be used to return either `getRMSE`, `getMAE`, or both results. 
...
We will again use the provided code to generate response and prediction variables for use in testing our wrapper funciton. 
...
Next, we will test the wrapper function using the data we just generated. We will use `getRMSE_MAE` once to output each statistic individually, then once to output both statistics together.
...
We will now repeat this process after replacing two of the response values with missing values using `NA_real_`. Instead of re-creating a vector with missing values, we will re-use the vector `resp_NA`. Recall, `resp_NA`'s values are:
...
We did not receive the expected "NA" responses for both outputs because we used our improved functions `getRMSE_2` and `getMAE_2` when writing `getRMSE_MAE`. Both of these helper functions have code written in them to account for missing values, resulting in the RMSE and MAE values being output instead of "NA".

Finally, we will test the function by using incorrect data type (i.e. a data frame instead of a vector). To do this, we will first generate a data frame using the response and predicted variables, and name it `resp_df`

```{r generate data frame for conditional response testing}
resp_df <- list("response" = resp, "predicted" = pred)
